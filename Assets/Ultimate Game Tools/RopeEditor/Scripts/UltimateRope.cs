...
public class UltimateRope : MonoBehaviour
{
    ...
    [RopePersistAttribute]  public int                  LinkSolverIterationCount        = 200;
    [RopePersistAttribute]  public float                LinkJointAngularXLimit          = 120.0f;
    [RopePersistAttribute]  public float                LinkJointAngularYLimit          = 120.0f;
    [RopePersistAttribute]  public float                LinkJointAngularZLimit          = 120.0f;
    [RopePersistAttribute]  public float                LinkJointSpringValue            = 20.0f;
    [RopePersistAttribute]  public float                LinkJointDamperValue            = 0.0f;
    [RopePersistAttribute]  public float                LinkJointMaxForceValue          = 20.0f;
    ...
    void CreateRopeJoints(bool bCheckIfBroken = false)
    {
        ...
        for(int nNode = 0; nNode < RopeNodes.Count; nNode++)
        {
            ...
            else
            {
                ...
                float fNodeDistance = Vector3.Distance(goSegmentStart.transform.position, goSegmentEnd.transform.position);

                if(fNodeDistance > RopeNodes[nNode].fLength)
                {
                    Debug.LogWarning( "Rope \"" + name + "\" segment " + nNode + " has a length shorter than the distance between the start node \"" + goSegmentStart.name + "\" and end node \"" + goSegmentEnd.name + "\".\n" +
